	...
	*/
	...
	public Contact getContactAt(int contactIndex) {
		...
		if (contactIndex < this.contacts.length && contactIndex >= this.size) {
			throw new ContactNotFoundException("연락처가 아직 채워지지 않은 인덱스입니다.");
		}
		...
		if (contactIndex >= this.contacts.length) {
			throw new ContactOutOfBoundsException("연락처 크기를 벗어나는 인덱스입니다. 올바른 인덱스를 요청하세요.");
		}
	...
	public void sort() {
		...
		for (int i = 0; i < this.size; i++) {
			...
			for (int j = i + 1; j < this.size; j++) {
				if ( this.contacts[minIdx].getProfileNumber() > this.contacts[j].getProfileNumber() ) {
					minIdx = j;
				}
			}
			
			if (minIdx != i) {
				Contact temp = this.contacts[minIdx];
				this.contacts[minIdx] = this.contacts[i];
